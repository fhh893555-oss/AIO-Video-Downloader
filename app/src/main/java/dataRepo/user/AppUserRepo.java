...
public final class AppUserRepo {
    ...
    private static void syncWithServer() {
        ...
        syncJob.cancel();
        syncJob.setMaxExecutionTimeMs(5000);
    ...
    private static void mergeServerUser(JSONObject serverData) {
        try {
            ...
            changed |= syncBooleanField(serverData, isPremiumField, localUser.isPremiumUser,
                    val -> localUser.isPremiumUser = val);
            changed |= syncBooleanField(serverData, hasReferralBonusField, localUser.hasReferralBonus,
                    val -> localUser.hasReferralBonus = val);
            changed |= syncBooleanField(serverData, isBannedField, localUser.isBanned, val -> localUser.isBanned = val);
            changed |= syncIntField(serverData, referralsCountField, localUser.totalReferralsCount,
                    val -> localUser.totalReferralsCount = val);
            if (changed) {
    ...
    private static boolean syncStringField(JSONObject data,
                                           String field,
                                           String current,
    ...
    private static boolean syncIntField(JSONObject data,
                                        String field, int current,
