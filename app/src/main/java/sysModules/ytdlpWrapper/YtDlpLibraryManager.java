...
public class YtDlpLibraryManager {
		...
		updateYtdlpTask.setBackgroundTask(progressCallback -> {
			try {
				...
				if (NetworkUtils.isInternetConnected()) {
					...
					YoutubeDL.UpdateChannel ytDlpUpdateChannel = getYtDlpUpdateChannel(deviceId);
	 ...
	 * </p>
	 *
	 * @param deviceId The unique identifier of the device requesting the update channel configuration.
	 * @return the resolved {@link YoutubeDL.UpdateChannel} to be used for the update process.
	 */
	private static YoutubeDL.UpdateChannel getYtDlpUpdateChannel(@NotNull String deviceId) {
		String cloudChannel = YtDlpChannelService.getActiveYtDlpChannel(deviceId);
